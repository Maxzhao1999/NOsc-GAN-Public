...
# %%
file_time = '20210105-154318'

index_list = [0, 51199, 102719, 70000, 98562, 98700, 15]
...
full_D_real_pred = []
full_D_fake_pred = []
full_pred_error_list = []

y = tf.cast(np.stack((th13s, dcps), axis=1), tf.float32)

for index in index_list:
    figure_loc = f"results/{file_time}/figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"
    #figure_loc = f"figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"
    os.mkdir(figure_loc)
    # figure_loc1 = figure_loc + '/D_and_error_graphs'
    # os.mkdir(figure_loc1)
...
for index in index_list:
    ...
    for epoch in list:
        ...
        pred_error_list.append(pred_error[index].mean())

        num = int(filenames[epoch].split('.')[1])

        DFP = discriminator.predict(tf.reshape(
            tf.concat([test[index], y[index]], axis=0), (1, 42)))
        DRP = discriminator.predict(tf.reshape(x_0[index], (1, 42)))

        D_fake_pred.append(DFP[0][0])
        D_real_pred.append(DRP[0][0])

        figure_loc = f"results/{file_time}/figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"
        #figure_loc = f"figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"

        plot_G_outputs(test[index], index, error_diff, bins, counts,
                       th13s, dcps, Full_Poisson_Error, num, figure_loc)

    full_pred_error_list.append(pred_error_list)
    full_D_real_pred.append(D_real_pred)
    full_D_fake_pred.append(D_fake_pred)


for i in range(len(full_D_real_pred)):

    index = index_list[i]
    figure_loc = f"results/{file_time}/figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"
    #figure_loc = f"figures/deeper_look_G_predictions_{th13s[index]}_{dcps[index]}"
    plot_D_predictions(
        list, full_D_real_pred[i], full_D_fake_pred[i], figure_loc)
    plot_errorgraph(list, full_pred_error_list[i], figure_loc)
